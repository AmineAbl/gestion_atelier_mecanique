<p align="center">
  <img src="MLD/atelier-mecanique.jpg" alt="Atelier mécanique" width="900" />
</p>
...
Si vous souhaitez démarrer rapidement le **back-end** avec les scripts fournis :

Depuis [back_end/](back_end/) :

```bash
composer run setup
```

Puis, pour lancer l’environnement de développement (serveur + assets, selon configuration) :

```bash
composer run dev
```

Ensuite, démarrez le **front-end** (voir section dédiée).
...
Exemples de variables usuelles :
...
> Option : si vous utilisez les assets Laravel (Vite), installez puis lancez le watcher :
>
> ```bash
> npm install
> npm run dev
...
> Si le front consomme une API, définissez la base URL via une variable d’environnement (par ex. `REACT_APP_API_BASE_URL`) et adaptez les appels HTTP en conséquence.

## Tests

- **Back-end** (depuis [back_end/](back_end/)) :

```bash
php artisan test
```

- **Front-end** (depuis [front_end/](front_end/)) :
